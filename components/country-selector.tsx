...
export default function CountrySelector() {
  ...
  return (
    <>
      <style>{`
        .cs-bg { background-image: url('/pc1.jpeg'); }
        @media (max-width: 767px) { .cs-bg { background-image: url('/iphone1.jpeg'); } }
        @media (min-width: 768px) and (max-width: 1023px) { .cs-bg { background-image: url('/Ipad1.jpeg'); } }
      `}</style>
    <div
      className={`cs-bg h-screen flex flex-col items-center justify-center absolute top-0 left-0 w-screen z-50 overflow-hidden`}
      style={{ backgroundSize: "100% 100%", backgroundRepeat: "no-repeat" }}
    >
      <div className="absolute inset-0 bg-black/30" />

      <div className="w-full max-w-md px-4 relative z-10" ref={dropdownRef}>
        <h1 className="text-2xl font-bold text-center mb-6 text-white drop-shadow-md">
